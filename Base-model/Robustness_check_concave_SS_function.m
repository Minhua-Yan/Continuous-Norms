...
pdf_Mig_Gen = zeros(gen, nbins);       pdf_Mig_Gen(1,:) = fFreq;             
pdf_SS_Gen = zeros(gen, nbins);        pdf_SS_Gen(1,:) = fFreq;              
pdf_Err_Gen = zeros(gen, nbins);       pdf_Err_Gen(1,:) = fFreq;             
fit_Soc_Gen = zeros(gen,nbins);                                             
for i = 1: nbins
        fit_Soc_Gen(1,i) = sum(exp(-S_inv *dis(i,:).*dis(i,:)/2).*fFreq);  
end
Mean_Gen = zeros(gen, 3);              Mean_Gen(1,:) = ones(1,3)*f;          
...
for g = 2:gen
  ...
  %% social selection as coordination
    ...
    fit_Max = 500;                                                           % adjust this value to control the range of distance that receives a non-zero payoff
    for i = 1: nbins
        fit_pair = -S_inv*dis(i,:).^2 + fit_Max;
        fit_pair(fit_pair<0) = 0;
        fit_Soc(i) = sum(fit_pair.*fFreq);  
