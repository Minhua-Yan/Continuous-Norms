...
m = 0.0001;               
z = 3;      f = 0;                                                                
U_sd = 1.5538;   S_inv = 1;    E_sd = 1;

if S_inv > 0
    V_sd= sqrt(E_sd^2 + sqrt(E_sd^4+E_sd^2/S_inv));                       % focal population equilibrium variance with only transission error and norm selection, see "eq V no mig calculation.pdf"                                                         % for simplicity and empirical realism, we assume the source population has the same S and E as the focal
elseif S_inv == 0
    error('S_inv must be positive, use "discretization_cat_constant_2parent" to investigate S_inv==0')
end
...
binRange= 10;                                                                % the trait range on one side of f we keep track of
...
z = sum(zFreq.*xM);                                                          % make sure this is close enough to the original value assigned to z
...
gen = 300;                                                                   % the number of generations to run the evolution for
