...
%   The only difference this file has compared to
%     "Coordination_migration.m" is that there are 2 social learning parents
%     and the child tries to learn the average of their norm traits. If the
%     average is not at the center of a base bin, before error, half of the children acquire
%     the trait of the bin on the left of the average, and the
%     other half of the children acquire the trait of the bin on the right of
%     the average.
...
Mean_Gen = zeros(gen, 4);                                                    % 1 column: mean after Mig, 2nd column: mean after SS, 3rd column: mean after SL with err
